...
public class AuthorDetailsView extends Application implements EventHandler<ActionEvent> {
	...
	public void start(Stage primaryStage, int userID, String prevPage) throws Exception {
		...
		authorName.setStyle("-fx-font: 20px Arial;" + "-fx-text-fill: #0076a3;" + "-fx-opacity: 0.6;");
		...
		if (selectedAuthor.getCommitteeMemberInfo().size() != 0) {
			StringBuilder str = new StringBuilder(selectedAuthor.getCommitteeMemberInfo()
					.get(confName.getSelectionModel().getSelectedItem()).toString());
			str.delete(0, 6);
		...
		TableColumn<ResearchPaper, Integer> articleYearCol = new TableColumn<ResearchPaper, Integer>(
				"Published in Year");
		...
		articleNameCol.setCellValueFactory(new PropertyValueFactory<ResearchPaper, String>("title"));
		articleYearCol.setCellValueFactory(new PropertyValueFactory<ResearchPaper, Integer>("year"));
		type.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<ResearchPaper, String>, ObservableValue<String>>() {
		...
		similarAuthor.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
		...
		back.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
				+ "-fx-background-color: linear-gradient(lightblue, white );");
		...
		bp.setTop(hlogout);
		// bp.setLayoutX(100);
		bp.setStyle("-fx-background-color:  linear-gradient(lightblue, white);" + " -fx-border-color: white;"
		...
		sp.setStyle("-fx-background-color:  linear-gradient(lightblue, white);" + " -fx-border-color: white;"
				+ " -fx-border-radius: 20;" + "-fx-padding: 10 10 10 10;" + "-fx-background-radius: 20;");
	...
	public void handle(ActionEvent event) {
		try {
			...
			if (event.getSource() == confName) {
				Set<String> value = selectedAuthor.getCommitteeMemberInfo()
						.get(confName.getSelectionModel().getSelectedItem());
				String display = "";
				for (String s : value) {
					display = display + s + "\n";
				}
