...
public class SearchResultView extends Application implements EventHandler<ActionEvent> {
	...
	private String style = "-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
	...
	public void start(Stage stage, ObservableList<Author> data, int userID) throws Exception {
		...
		authorDetails.setStyle("-fx-selection-bar: lightblue; ");
		// Table Filter
		...
		filterChoice
				.setItems(FXCollections.observableArrayList("Author Name", "No of Research Papers", "Past Experience"));
		...
		addButton.setId("add");
		addButton.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
		...
		logout.setId("add");
		logout.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
		...
		viewSelectedList.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
				+ "-fx-background-color: linear-gradient(lightblue, white );");
		...
		// finalLayout.setStyle("-fx-background-color: WHITESMOKE ; -fx-padding:
		// 10;");
		finalLayout.setStyle("-fx-background-color:  linear-gradient(lightblue, white);" + " -fx-border-color: white;"
				+ " -fx-border-radius: 20;" + "-fx-padding: 10 10 10 10;" + "-fx-background-radius: 20;");
	...
	public void handle(ActionEvent event) {
		try {
			...
			if (event.getSource() == addButton) {
				...
				/*
				 * HSSFWorkbook workbook = new HSSFWorkbook(); HSSFSheet
				 * spreadsheet = workbook.createSheet("sample"); HSSFRow row
				 * =null; System.out.println(authorDetails.getItems().size());
				 * System.out.println(authorDetails.getColumns().size()); row=
				 ...
				 * for(int i=1;i<=authorDetails.getItems().size();i++){ row=
				 * spreadsheet.createRow(i); for(int j=0; j<
				 * authorDetails.getColumns().size();j++){
				 * if(authorDetails.getColumns().get(j).getCellData(i)==null)
				 * break;
				 * row.createCell(j).setCellValue(authorDetails.getColumns().get
				 * (j).getCellData(i).toString()); } } FileOutputStream out =
				 * new FileOutputStream("example.xls"); workbook.write(out);
				 * out.close();
			...
			if (event.getSource() == applyFilter) {
				...
				if (filterVal.isEmpty()) {
					...
					resultLbl.setText(
							filterauth.size() + " authors fetched for past experience in years : " + filterVal);
				}
	...
	private void setDataInTable(ObservableList<Author> data) {
		...
		// Set Column Value
		authorNameCol.setCellValueFactory(new PropertyValueFactory<Author, String>("name"));
			...
			TableCell<Author, String> cell = new TableCell<Author, String>() {
				...
				public void updateItem(String item, boolean empty) {
					...
					if (item != null) {
						...
						text.setStyle(" -fx-opacity: 1;" + " -fx-font-family: \"verdena\";" + " -fx-font-size: 10pt;"
								+ " -fx-fill: #1398c8;" + " -fx-text-wrap: true;" + " -fx-padding: 5px 30px 5px 5px;"
		...
		researchPaperCol.setCellValueFactory(
				new Callback<TableColumn.CellDataFeatures<Author, Integer>, ObservableValue<Integer>>() {
