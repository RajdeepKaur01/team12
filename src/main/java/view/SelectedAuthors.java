...
public class SelectedAuthors extends Application implements EventHandler<ActionEvent> {
	...
	public void start(Stage primaryStage, int userID) {
		...
		delbtn.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
		...
		savebtn.setStyle("-fx-background-radius: 30, 30, 29, 28;" + "-fx-padding: 3px 10px 3px 10px;"
				+ "-fx-background-color: linear-gradient(lightblue, white );");
		...
		welcome.setStyle("-fx-font: 20px Arial;" + "-fx-text-fill: #0076a3;" + "-fx-opacity: 0.6;");
		...
		selectedStage.show();
		layout.setStyle("-fx-background-color:  linear-gradient(lightblue, lightblue);" + " -fx-border-color: white;"
	...
	private void setDataInTable(ObservableList<Author> data) {
		...
		pastExpCol.setCellValueFactory(new PropertyValueFactory<Author, Integer>("pastExperienceYrs"));

		/*
		 * pastExpCol.setCellValueFactory(new
		 * Callback<TableColumn.CellDataFeatures<Author,String>,
		 * ObservableValue<String>>() {
		 * 
		 * @Override public ObservableValue<String>
		 * call(CellDataFeatures<Author, String> p) { //
		 * System.out.println(p.getValue().getCommitteeMemberInfo().size());
		 * if(p.getValue().getCommitteeMemberInfo()==null ||
		 * p.getValue().getCommitteeMemberInfo().size() == 0) return new
		 * SimpleStringProperty("No Experience");
		 * 
		 * return new SimpleStringProperty("" +
		 * p.getValue().getCommitteeMemberInfo().size());
		 * 
		 * 
		 * } });
		 */
